 ...
 * Priority: ANTHROPIC_API_KEY (independent quota) > CLAUDE_CODE_OAUTH_TOKEN
 * (subscription token from `claude setup-token`, needs the oauth beta header).
 */
export function makeClient(): { client: Anthropic; isOAuth: boolean } {
  const apiKey = process.env.ANTHROPIC_API_KEY?.trim();
  const oauth = (
    process.env.CLAUDE_CODE_OAUTH_TOKEN ?? process.env.ANTHROPIC_AUTH_TOKEN
  )?.trim();

  if (apiKey) return { client: new Anthropic({ apiKey }), isOAuth: false };
  if (oauth) return { client: new Anthropic({ authToken: oauth }), isOAuth: true };

  throw new Error(
    "No credentials found. Set CLAUDE_CODE_OAUTH_TOKEN (run `claude setup-token`) " +
...
function betaHeader(isOAuth: boolean, webFetch: boolean): Record<string, string> | undefined {
  const betas: string[] = [];
  if (isOAuth) betas.push("oauth-2025-04-20");
  if (webFetch) betas.push("web-fetch-2025-09-10");
...
export async function streamText(args: StreamArgs): Promise<string> {
  const { client, isOAuth } = makeClient();
    ...
    const stream = client.messages.stream(
      {
        ...
        ...(useTools ? { tools: webTools() } : {}),
      },
      { headers: betaHeader(isOAuth, useTools) },
    );
  ...
  for (;;) {
    try {
      const res = await client.messages.create(
        ...
        { headers: betaHeader(isOAuth, false) },
      );
